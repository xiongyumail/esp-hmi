...
static void lcd_cam_task(void *arg)
{
    ...
    gc9a01.set_index(0, 0, LCD_WIDTH - 1, LCD_HIGH - 1);
    lcd_cam.lcd.swap_data(false);
    gc9a01.write_data((uint8_t *)img_buf, LCD_WIDTH * LCD_HIGH * 2);
    ...
    while (1) {
        ...
        if (img) {
            ...
            gc9a01.set_index(0, 0, w - 1, h - 1);
            gc9a01.write_data(img, w * h * sizeof(uint16_t));
            free(img);
        }
#else
#if CAM_JPEG_ENCODE
...
#else
        gc9a01.set_index(0, 0, CAM_WIDTH - 1, CAM_HIGH - 1);
        gc9a01.write_data(cam_buf, CAM_WIDTH * CAM_HIGH * 2);
#endif
#endif
