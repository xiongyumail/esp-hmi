...
static void lcd_cam_task(void *arg)
{
    ...
    while (1) {
        ...
        if (img) {
            ...
            st7789.set_index(0, 0, w - 1, h - 1);
            st7789.write_data(img, w * h * sizeof(uint16_t));
            free(img);
        }
#else
#if CAM_JPEG_ENCODE
...
#else
        st7789.set_index(0, 0, CAM_WIDTH - 1, CAM_HIGH - 1);
        st7789.write_data(cam_buf, CAM_WIDTH * CAM_HIGH * 2);
#endif
#endif
