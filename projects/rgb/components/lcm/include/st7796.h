...
typedef struct st7796_handle_struct st7796_handle_t;
...
typedef struct {
    struct {
        struct {
            void (*run)(st7796_handle_t *handle);
            void (*stop)(st7796_handle_t *handle);
        } mst;
        struct {
            void (*run)(st7796_handle_t *handle);
            void (*stop)(st7796_handle_t *handle);
    ...
    struct {
        struct {
            void (*write) (st7796_handle_t *handle, uint8_t *data, size_t len);
        } mst;
        struct {
            void (*set_index)(st7796_handle_t *handle, uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end);
            void (*write) (st7796_handle_t *handle, uint8_t *data, size_t len);
        } slv;
    } data;
} st7796_port_t; 

struct st7796_handle_struct {
    void *self;
    st7796_ctrl_t *ctrl; // 寄存器
    st7796_port_t *port; // 驱动器
}; /*!< lcd handle */
